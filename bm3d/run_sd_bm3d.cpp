...
,   block_matching_ftype blk_method1       
,   sdbm3dPtype    parmsStep1
,   block_matching_ftype blk_method2       
,   sdbm3dPtype    parmsStep2        
){

            
    //! Parameters
    const unsigned nHard = 16; //! Half size of the search window 16
    const unsigned nWien = 16; //! Half size of the search window 16
    const unsigned kHard = (parmsStep1.transform == BIOR || sigma < 40.f ? 8 : 12); //! Must be a power of 2 if tau_2D_hard == BIOR 8/12
    const unsigned kWien = (parmsStep2.transform == BIOR || sigma < 40.f ? 8 : 12); //! Must be a power of 2 if tau_2D_wien == BIOR 8/12
    const unsigned NHard = 16; //! Must be a power of 2 16
    const unsigned NWien = 32; //! Must be a power of 2 32
    const unsigned pHard = 3; //3
    const unsigned pWien = 3; //3
    ...
    if (color_space_transform(img_noisy, RGB, width, height, chnls, true)
...
#endif
    //mexPrintf(" Number of threads which will be used: %d \n", nb_threads);
#ifdef _OPENMP
    mexPrintf(" (real available cores:  %d \n", omp_get_num_procs());   
#endif
    ...
    if (nb_threads == 1)
    {
        ...
        symetrize(parmsStep1.filtered_img, parmsStep1.filtered_img, width, height, chnls, nHard);
        symetrize(parmsStep2.filtered_img, parmsStep2.filtered_img, width, height, chnls, nHard);

        //! Allocating Plan for FFTW process
        if (parmsStep1.transform == DCT)
        {
        ...
        bm3d_1st_step(sigma, img_sym_noisy, img_sym_basic, w_b, h_b, chnls, nHard,
                      kHard, NHard, pHard, &plan_2d_for_1[0], &plan_2d_for_2[0], 
                      &plan_2d_inv[0], blk_method1, parmsStep1);
        ...
        bm3d_2nd_step(sigma, img_sym_noisy, img_sym_basic, img_sym_denoised,
                w_b, h_b, chnls, nWien, kWien, NWien, pWien, &plan_2d_for_1[0], 
                &plan_2d_for_2[0], &plan_2d_inv[0], blk_method2, parmsStep2);
        //mexPrintf("done \n");  
    ...
    if (color_space_transform(img_denoised, RGB, width, height, chnls, false)
        != EXIT_SUCCESS) return EXIT_FAILURE;
    if (color_space_transform(img_noisy, RGB, width, height, chnls, false)
        != EXIT_SUCCESS) return EXIT_FAILURE;
    if (color_space_transform(img_basic, RGB, width, height, chnls, false)
        != EXIT_SUCCESS) return EXIT_FAILURE;

    //! Free Memory
    if (parmsStep1.transform == DCT || parmsStep2.transform == DCT)
...
,   block_matching_ftype blk_method        
,   sdbm3dPtype parms
){       
    //! Estimatation of sigma on each channel
    vector<float> sigma_table(chnls);
    if (estimate_sigma(sigma, sigma_table, chnls, RGB) != EXIT_SUCCESS)
        return;

    //! Parameters initialization
    float    tauMatch_ec = (chnls == 1 ? 3.f : 1.f) * (sigma_table[0] < 35.0f ? 2500 : 5000); //! threshold used to determinate similarity between patches
    ...
    vector<float> homo_table(width * height,-1.0f);
    
    blk_method(patch_table, img_noisy, width, height, kHard, NHard, nHard, pHard, tauMatch_ec, sigma, parms); 
    ...
    for (unsigned ind_i = 0; ind_i < row_ind.size(); ind_i++)
    {
        ...
        for (unsigned ind_j = 0; ind_j < column_ind.size(); ind_j++)
        {
            ...
            ht_filtering_hadamard(group_3D, hadamard_tmp, nSx_r, kHard, chnls, sigma_table,
                                                    parms.lambdaHard3D, weight_table, !parms.use_std);
...
,   sdbm3dPtype   parms
    ...
    //! Parameters initialization
    float tauMatch_ec = (sigma_table[0] < 35.0f ? 400 : 3500); //! threshold used to determinate similarity between patches
    ...
    blk_method(patch_table, img_basic, width, height, kWien, NWien, nWien, pWien, tauMatch_ec, sigma, parms);
    ...
    for (unsigned ind_i = 0; ind_i < row_ind.size(); ind_i++)
    {
        ...
        for (unsigned ind_j = 0; ind_j < column_ind.size(); ind_j++)
        {
            ...
            wiener_filtering_hadamard(group_3D_img, group_3D_est, tmp, nSx_r, kWien,
                                            chnls, sigma_table, weight_table, !parms.use_std);
